...
path = Path("data/phase3_scored.csv")
if not path.exists():
    path = Path("data/results/phase3_scored.csv")
...
if df is None:
    ...
    # Re-save as clean UTF-8 so future scripts don't hit this issue
    clean_path = path.parent / "phase3_scored_utf8.csv"
    ...
    print(df[["model", "prompt_id", "score_correctness", "score_coherence", "score_instruction", "score_overall"]].head(10).to_string(index=False))
